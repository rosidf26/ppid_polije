...
<head>
  <!-- ini head --> @include('frontpage.templates.head')
</head>

<body>
  <div class="body">
    <!-- ini header --> @include('frontpage.templates.header')
    <div role="main" class="main">
      <!-- ini header --> @include('frontpage.sections.page_title')
      <div class="container py-2">
        <div class="row">
          <div class="col-lg-3">
            <aside class="sidebar">
              <form action="{{ route('search.announce') }}" method="get">
              ...
              <h5 class="font-weight-bold pt-4">Tags</h5> @if(isset($all_tags) && $all_tags->count() > 0)
                <ul class="nav nav-list flex-column mb-5"> @foreach ($all_tags as $tg) <li class="nav-item">
                  <a class="nav-link {{ request()->segment(3) == $tg->slug ? 'active' : '' }}"
                    href="{{ route('tag.filter_announce', $tg->slug) }}"> {{ $tg->name }}
                    ({{ $tg->announce_count }})</a>
          ...
          <div class="row flex-grow-1">
            <div class="col-auto pr-0">
              <div class="post-date detail">
                <span
                  class="day text-color-dark font-weight-extra-bold">{{ \Carbon\Carbon::parse($value->date)->translatedFormat('d') }}</span>
