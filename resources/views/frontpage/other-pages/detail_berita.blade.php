              ...
              @if(isset($all_tags) && $all_tags->count() > 0)
                <ul class="nav nav-list flex-column mb-5">
                  @foreach ($all_tags as $tg)
                    <li class="nav-item"><a class="nav-link {{ request()->segment(3) == $tg->slug ? 'active' : '' }}"
                        href="{{ route('tag.filter_news', $tg->slug) }}"> {{ $tg->name }} ({{ $tg->articles_count }})</a>
                    </li>
                  @endforeach
                </ul>
              @endif
            ...
            <div class="blog-posts single-post">
              <article class="post post-large blog-single-post border-0 m-0 p-0">
                <div class="post-image ml-0">
                  <a href="#">
                    <img src="{{ $article->image ? url($article->image) : url('frontpage/img/blog/wide/blog-11.jpg') }}"
                ...
                <div class="post-content ml-0">
                  ...
                  <div class="post-meta">
                    <span><i class="far fa-user"></i> By <a>Admin</a>
                    </span>
