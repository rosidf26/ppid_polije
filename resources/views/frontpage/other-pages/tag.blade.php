							...
							@if(isset($all_tags))
								<ul class="nav nav-list flex-column mb-5">
									@foreach ($all_tags as $tg)
										<li class="nav-item"><a
												class="nav-link {{ isset($tag) && $tag->slug == $tg->slug ? 'active' : '' }}"
												href="{{ route('tag.filter_news', $tg->slug) }}"> {{ $tg->name }}
												({{ $tg->articles_count }}) </a></li>
									@endforeach
								</ul>
							@endif
							...
							@foreach($articles as $index => $value)
								<article class="post post-medium">
										...
										<div class="col-lg-5">
											<div class="post-image">
												<a href="{{ url($value->category->slug . '/' . $value->slug) }}">
													<img src="{{ $value->image ? url($value->image) : url('frontpage/img/blog/medium/blog-11.jpg') }}"
										...
										<div class="col-lg-7">
											<div class="post-content">
												<h2 class="font-weight-semibold pt-4 pt-lg-0 text-5 line-height-4 mb-2"><a
														href="{{ url($value->category->slug . '/' . $value->slug) }}">{{ \Illuminate\Support\Str::limit($value->title, 50) }}</a>
												</h2>
									...
									<div class="row">
										<div class="col">
											<div class="post-meta">
												<span><i class="far fa-calendar-alt"></i> {{ tgl_indo($value->date) }}
												</span>
												...
												<span class="d-block d-sm-inline-block float-sm-right mt-3 mt-sm-0"><a
														href="{{ url($value->category->slug . '/' . $value->slug) }}"
														class="btn btn-xs btn-light text-1 text-uppercase">Baca
														Selengkapnya</a></span>
											</div>
										</div>
									</div>
