    ...
    <div class="body">
      <!-- ini header --> @include('frontpage.templates.header')
	   <div role="main" class="main">
        <!-- ini header --> @include('frontpage.sections.page_header')
          ...
          <div class="row">
            <div class="col-lg-3">
              <aside class="sidebar">
                ...
                <h5 class="font-weight-bold pt-4">Tags</h5> @if(isset($all_tags) && $all_tags->count() > 0) <ul class="nav nav-list flex-column mb-5"> @foreach ($all_tags as $tg) <li class="nav-item">
                    <a class="nav-link {{ request()->segment(3) == $tg->slug ? 'active' : '' }}" href="{{ route('tag.filter_announce', $tg->slug) }}"> {{ $tg->name }} ({{ $tg->announce_count }})</a>
